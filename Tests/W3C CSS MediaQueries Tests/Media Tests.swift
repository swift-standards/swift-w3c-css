...
struct `Media - Resolution Queries` {
    @Test func `resolution feature methods create correct media queries`() throws {
        ...
        #expect(Media.minResolution(try Resolution.dppx(2)).rawValue == "@media (min-resolution: 2dppx)")
    }
}
...
struct `Media - Predefined Queries` {
    @Test func `predefined viewport queries`() {
        ...
        #expect(Media.tablet.rawValue == "@media only screen and (min-width: 768px) and (max-width: 1024px)")
    }
    ...
    @Test func `predefined device queries`() {
        #expect(Media.retina.rawValue == "@media (-webkit-min-device-pixel-ratio: 2), (min-resolution: 192dpi)")
        #expect(Media.smallMobile.rawValue == "@media only screen and (max-width: 320px)")
        #expect(Media.largeMobile.rawValue == "@media only screen and (min-width: 321px) and (max-width: 767px)")
